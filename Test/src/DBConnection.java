...
public class DBConnection {
	public void connection(ArrayList<String> saveList) {
		...
		String url = "jdbc:mysql://1.240.181.56:3306/sgyy?characterEncoding=UTF-8&amp;serverTimezone=UTC&useSSL=false";
		...
		try (Connection conn = DriverManager.getConnection(url, "sgyy", "62066407")) {
			...
			for (int i = 0; i < saveList.size(); i++) {
				...
				String vair = itemArr[1]; // 품 종

				String sql = "INSERT INTO ingredientssubject (subjectUUID, name) VALUES (UNHEX(REPLACE(UUID(), '-', '')), '"
						+ vair + "')";
