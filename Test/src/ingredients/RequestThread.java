...
public class RequestThread extends Thread {
	...
	static HashMap<String, ArrayList<Variety>> hashMap = new HashMap<String, ArrayList<Variety>>();
	...
	public void run() {
		...
		String item = queryArr[0]; // 폼목
		String vair = queryArr[1]; // 품종
		...
		try {
			...
			if (doc.selectFirst("ul[id='idProductImg']") != null) {
				...
				for (Element liListItem : liListItemArr) {
					...
					System.out.println(variety.getName()+", "+query);
					if(variety.getName().contains(query)) {
						varietyList.add(count, variety);
						System.out.println(variety+", "+query);
						count++;
					}


					hashMap.put(query, varietyList);
					System.out.println(hashMap.get(query));
